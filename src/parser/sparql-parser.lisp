...
(defun build-query-expression-new (instans clauses)
  ...
  (let ((query-form (getf clauses :query-form)))
    (case query-form
      ...
      (INSERT-DATA
       (inform "INSERT-DATA: ~S" clauses)
       (flet ((check-triple (triple) (when (some #'sparql-var-p triple) (sparql-parse-error "Variables not allowed in INSERT DATA: ~A" triple))))
	 (loop for item in (cddr clauses)
	       when (eq (first item) 'GRAPH)
	       do (let ((graph (second item)))
		    (when (sparql-var-p graph)
		      (sparql-parse-error "Variables not allowed in INSERT DATA: ~A" graph))
