...
(defun do-select-tests (forms input-file)
  (let* ((attrs (rest forms))
    ...
    (let ((instans (make-instance 'instans :name name))
	  (triples (rest triplesdata)))
      (compile-sparql-algebra-expr select instans)
      (setf (instans-input-function instans) #'(lambda () (and triples (pop triples))))
      (let* ((bindings (instans-bindings instans))
