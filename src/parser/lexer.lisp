...
(define-class triple-lexer (abstract-sparql-triple-lexer) ())

(define-class turtle-lexer (triple-lexer) ())

(define-class trig-lexer (triple-lexer) ())
...
(defmethod make-keyword-table ((this triple-lexer))
  (declare (ignorable this))
  (let ((table (make-binding-table)))
    (loop for (keyword . terminal) in '(("a" . A-TERMINAL)
					("BASE" . BASE-TERMINAL)
	  ...
	  unless (and (turtle-lexer-p this) (eq terminal 'GRAPH-TERMINAL))
	  do (setf (cdr binding) terminal))
    table));;; Use these to get an apropriate lexer

(defmethod get-input-token ((lexer triple-lexer))
...
(defun eat-number (lexer buf) ; We saw a digit (not .). There may be a preceding '+' or '-' in buf
  ...
  (loop for ch = (peekch lexer)
	...
	do (chbuf-put-char buf (get-char lexer))
		      ...
		      (t (return-input-token lexer (cond ((triple-lexer-p lexer) 'INTEGER-TERMINAL)
...
(defun eat-fraction (lexer buf) ; There may be a sign and integral part. Last of buf is #\., which we have already consumed. We are looking at a digit.
  (loop for ch = (peekch lexer)
	...
	do (chbuf-put-char buf (get-char lexer))
		      ...
		      (t (return-input-token lexer (cond ((triple-lexer-p lexer) 'DECIMAL-TERMINAL)
...
(defun eat-exponent (lexer buf) ;; (peekch lexer) in "eE"
  ...
  (loop for ch = (peekch lexer)
	while (digit-char-p* ch)
	do (chbuf-put-char buf (get-char lexer))
	finally (return-input-token lexer (cond ((triple-lexer-p lexer) 'DOUBLE-TERMINAL)
...
(defun eat-at (lexer)
  (loop with buf = (empty-chbuf #\@)
	...
	do (chbuf-put-char buf (get-char lexer))
	finally (cond ((triple-lexer-p lexer)
		       (let ((binding (resolve-keyword lexer buf)))
			 (cond ((null binding)
...
(defun eat-iri (lexer) ; we saw '<'
  (loop with buf = ;(if (triple-lexer-p lexer) 
       (empty-chbuf)
	for ch = (peekch lexer)
  ...
  (multiple-value-bind (type lexem)
	...
	(progn
	  ...
	  (cond ((null (peekch lexer))
		...
		((get-char-if-looking-at lexer #\!)
		 (cond ((triple-lexer-p lexer) (lexer-error lexer "Unrecognized input-token ~C" #\!))
		...
		((get-char-if-looking-at lexer #\+)
		 (cond ((get-char-if-looking-at lexer #\.)
			(cond ((digit-char-p* (peekch lexer)) (eat-fraction lexer (empty-chbuf #\.)))
			      ((triple-lexer-p lexer) (lexer-error lexer "Unrecognized input-token ~C" (peekch lexer)))
