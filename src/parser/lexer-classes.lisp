...
(defgeneric lexer-punctuation (abstract-sparql-rdf-lexer)
  (:method ((this sparql-lexer))
    '(("!" !-TERMINAL)
      ("!=" !=-TERMINAL)
      ("&&" &&-TERMINAL)
...
(defgeneric input-token-to-string (lexer input-token)
  (:method ((this sparql-lexer) input-token)
    (let* ((token-type (input-token-type input-token)))
      (or (first (find-if #'(lambda (item) (eq token-type (second item))) (lexer-keywords this)))
	  ...
	  (let ((value (input-token-value input-token)))
	    (case token-type
	      ...
	      (DOUBLE-TERMINAL (format nil "~e" value))
	      (DOUBLE_NEGATIVE-TERMINAL (format nil "~e" value))
	      (DOUBLE_POSITIVE-TERMINAL (format nil "~@e" value))
	      (INTEGER-TERMINAL (format nil "~D" value))
	      (INTEGER_NEGATIVE-TERMINAL (format nil "~D" value))
	      (INTEGER_POSITIVE-TERMINAL (format nil "~@D" value))
	      (IRIREF-TERMINAL (format nil "<~A>" (rdf-term-as-string value)))
	      (LANGTAG-TERMINAL (format nil "@~A" value))
	      (PNAME_LN-TERMINAL (format nil "<~A>" (rdf-term-as-string (pname-to-iri this (first value) (second value)))))
	      (PNAME_NS-TERMINAL (format nil "<~A>" (rdf-term-as-string (pname-to-iri this (first value) ""))))
	      (STRING_LITERAL1-TERMINAL (format nil "'~A'" value))
...
;; (defgeneric input-token-token-to-string (sparql-lexer input-token)
;;   (:method ((this sparql-lexer) input-token)
;;     (
	   ...
	   (let* ((input-token2 (next-input-token lexer))
	     ...
	     (cond ((and (eq type1 'INSERT-TERMINAL) (eq type2 'DATA-TERMINAL))
		   ...
		   ((and (eq type1 '|(-TERMINAL|) (eq type2 '|)-TERMINAL|))
		    (make-input-token :type 'NIL-TERMINAL :value (make-rdf-iri :string "http://www.w3.org/1999/02/22-rdf-syntax-ns#nil")))
		   ((and (eq type1 '|[-TERMINAL|) (eq type2 '|]-TERMINAL|))
		    (make-input-token :type 'ANON-TERMINAL :value "ANON"))
		   (t
		    (setf (lexer-buffered-input-token lexer) input-token2)
