...
(define-class rete-output-processor ()
  ((instans :accessor rete-output-processor-instans :initarg :instans)
...
(define-class rete-trig-output-processor (rete-construct-output-processor)
  ((current-graph :accessor rete-trig-output-processor-current-graph :initform nil)
   ...
   (prefixes-written-p :accessor rete-trig-output-processor-prefixes-written-p :initarg :prefixes-written-p :initform nil)
...
(defun create-construct-stream-output-processor (instans output-name output-type)
  (let* ((stream (cond ((or (null output-name) (string= "-" output-name)) *standard-output*)
    ...
    (case output-type
      ((:ttl :turtle) (make-instance 'rete-turtle-output-processor :instans instans :output-name output-name :output-writer output-writer))
      (:trig (make-instance 'rete-trig-output-processor :instans instans :output-name output-name :output-writer output-writer))
      (:nt (make-instance 'rete-nt-output-processor :instans instans :output-name output-name :output-writer output-writer))
      (:nq (make-instance 'rete-nq-output-processor :instans instans :output-name output-name :output-writer output-writer))
...
(defgeneric flush-construct-output (rete-construct-output-processor)
  ...
  (:method ((this rete-trig-output-processor))
    (let ((instans (rete-output-processor-instans this))
	  (writer (rete-output-processor-output-writer this)))
      (maybe-write-prefixes instans this)
      (write-output-trie writer instans (rete-trig-output-processor-spol-trie this))
...
(defgeneric write-output-trie (rete-output-writer instans trie)
  (:method ((this rete-stream-output-writer) (instans instans) trie)
...
;					 do (inform "o = ~S" o)
			 ...
			 do (incf indent (- (length s-string)))
			 do (format stream " .~%"))))))
  (:method ((this rete-agent-output-writer) (instans instans) trie)
    (let* ((msg (list nil))
	   (tail msg))
...
(defgeneric maybe-write-prefixes (instans rete-output-processor)
  (:method ((instans instans) (processor rete-trig-output-processor))
    (let ((writer (rete-output-processor-output-writer processor)))
      (when (and (typep writer 'rete-stream-output-writer)
		 (instans-encode-prefixes-p instans)
		 (not (rete-trig-output-processor-prefixes-written-p processor)))
	(loop for (k . v) in (instans-prefixes instans)
	      when (string-equal k "BASE")
	      do (format (rete-stream-output-writer-stream writer) "BASE <~A>~%" v)
	      else
	      do (format (rete-stream-output-writer-stream writer) "PREFIX ~A: <~A>~%" k v))
	(setf (rete-trig-output-processor-prefixes-written-p processor) t))))
  (:method ((instans instans) (processor rete-output-processor))
