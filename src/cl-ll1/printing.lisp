...
(defun print-grammar (grammar &optional (stream *error-output*))
...
(defun print-firsts (grammar &optional (stream *error-output*))
  (let ((terminals (grammar-terminals grammar)))
    (when (grammar-nonterminal-firsts grammar)
      (format stream "~%")
      (dolist (f (grammar-nonterminal-firsts grammar))
...
(defun print-followers (grammar &optional (stream *error-output*))
  (let ((terminals (grammar-terminals grammar)))
    (when (grammar-followers grammar)
      (format stream "~%")
      (dolist (f (grammar-followers grammar))
...
(defun print-ll-parse-table (grammar &optional (stream *error-output*))
  (let* ((terminals (append (grammar-terminals grammar) (list '$)))
	 (nonterminals (grammar-nonterminals grammar))
	 (parse-table (grammar-ll1-table grammar))
	 (productions (grammar-productions grammar))
	 (*print-pretty* nil)
	 (max-width 0))
...
(defun ll-parse-table2csv (grammar &optional (stream *error-output*))
  (let* ((terminals (append (grammar-terminals grammar) (list '$)))
	 (nonterminals (grammar-nonterminals grammar))
	 (parse-table (grammar-ll1-table grammar))
	 (*print-pretty* nil))
...
(defun print-ll-parse (parse grammar input &optional (stream *error-output*) show-productions-p)
  (let* ((terminals (grammar-terminals grammar))
...
(defun print-ll-derivation (parse grammar input &optional (stream *error-output*) show-productions-p)
  (declare (ignorable show-productions-p))
  (let* ((terminals (grammar-terminals grammar))
	 (start (format nil "~A" (grammar-start-symbol grammar)))
