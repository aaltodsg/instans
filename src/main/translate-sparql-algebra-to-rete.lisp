    ...
    (labels ((replace-exists-by-vars (e)
		 ...
		 (labels ((walk (x)
			    (cond ((consp x)
				  ...
				  (t x))))
		   (let ((new-expr (walk e)))
;		     (barf "replace ~A ->~%  ~A~%  ~A~%  ~A" e new-expr exists-list var-list)
		 ...
		 (barf "make-or-share-instance ~S (~{~A~^ ~})" type args)
		 (let ((prev (or (getf args :prev) (getf args :beta))))
		   ;; (barf "  network-nodes = " (network-nodes network))
...
; Check this			      (or (zerop level) (member type '(triple-pattern-node datablock-node alpha-memory)))
	     ...
	     (translate (expr prev dataset)
	       (let ((op (first expr))
		     (args (rest expr)))
		 ; (barf "translate expr = ~S, prev = ~S, dataset = ~S~%op = ~S,~%args=~S~%" expr prev dataset op args)
		 (case op
		   (ZERO-PATTERN
		    (assert (null prev))
