    ...
    (labels ((replace-exists-by-vars (e)
	     ...
	     (equal-value (v1 v2)
		   ...
		   (cond ((consp v1)
			 ...
			 ((term-or-value-p v1)
			  (and (term-or-value-p v2) (sparql-call "sameTerm" v1 v2))))))
		     )
;		 (inform "equal-value ~S ~S -> ~S" v1 v2 r)
		 ...
		 (case op
		   ...
		   (DELETE-INSERT (let ((where-clause (getf args :where))
				    ...
				    (inform "DELETE-INSERT: args ~S" args)
				    (inform "insert-clause: ~S" insert-clause)
...
(defun translate-template (netvar template op)
  (multiple-value-bind (graph triple-patterns)
    ...
    (loop for triple-pattern in triple-patterns
	  collect `(,op ,netvar ,@(loop for term in triple-pattern collect (cond ((sparql-var-p term) (sparql-var-lisp-name term))
										 ((rdf-iri-p term) `(create-iri ,(rdf-iri-string term)))
										 ((rdf-literal-p term) `(create-rdf-literal
										 ...
										 ((rdf-blank-node-p term) `(create-blank-node ,(uniquely-named-object-name term)))
										 ((datetimep term) `(create-datetime ,(datetime-canonic-string term)))
										 ((xsd-value-p term) term)
										 (t (error* "Cannot translate ~S" term))))
