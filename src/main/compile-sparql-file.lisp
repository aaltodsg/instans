...
;		      (warn "bind-lambda = ~S" bind-lambda)
		   ...
		   ((modify-node-p node)
		    (let ((delete-lambda `(lambda (,(first (modify-delete-template node))
			  ...
			  (insert-lambda `(lambda (,(first (modify-insert-template node))
						   ,@(mapcar #'second (modify-insert-parameters node)))
					    ,@(rest (modify-insert-template node)))))
		      (setf (modify-delete-lambda node) delete-lambda)
		      (setf (modify-delete-func node) (and (rest (modify-delete-template node)) (compile nil delete-lambda)))
		      (setf (modify-insert-lambda node) insert-lambda)
		      (setf (modify-insert-func node) (and (rest (modify-insert-template node)) (compile nil insert-lambda)))))))
