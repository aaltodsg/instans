...
;	       (barf "di= ~S" datatype/lang-item)
    ...
    (let* ((parsed (with-open-file (s filename)
      ...
      (barf "Parsing ~A returned:~%~S" filename parsed)
      (barf "Translated = ~S" translated)
      translated))))
...
(defun parse-srx-files (pathname-pattern)
  (loop for file in (directory pathname-pattern)
	...
	do (cond ((slot-boundp tr 'boolean)
		  (barf "Boolean value ~A" (sparql-query-results-solutions tr)))
		 ((slot-boundp tr 'results)
		  (loop for s in (sparql-query-results-solutions tr)
			do (barf "Solution ~A" s)))
		 (t (barf "No boolean nor results")))))
