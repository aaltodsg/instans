...
(defmethod make-keyword-table ((this n-statements-lexer))
  (make-binding-table))

(defmethod make-keyword-table ((this trig-lexer))
  (declare (ignorable this))
  (let ((table (make-binding-table)))
    (loop for (keyword . terminal) in '(("a" . A-TERMINAL)
					("BASE" . BASE-TERMINAL)
...
(defun skip-whitespace-and-comments-old (lexer)
  (loop for ch = (peekch lexer)
	do (cond ((null ch) (return nil))
	     ...
	     (let ((num1234 (+ ch4 (* 16 (+ ch3 (* 16 (+ ch2 (* 16 ch1)))))))
	       ...
	       (cond ((setf ch5 (get-if-looking-at-hex-digit))
		      (cond ((setf ch6 (get-if-looking-at-hex-digit))
			     (code-char (+ ch6 (* 16 (+ ch5 (* 16 num1234))))))
			    (t
