...
TMP=$$tmp
cat <<EOF >$TMP
$INSTANS --prefix-encoding=true --construct-output=$POSTSTATELESS -r construct-EPA0.rq --rdf-operations=add:execute --allow-rule-instance-removal=false --input-single=$SOURCE
...
# $INSTANS --prefix-encoding=true --construct-output=$EVENTCOUNTS -r `pwd`/construct-EPA6.rq --rdf-operations=add:execute:remove:execute:flush -g http://instans.org/source${TIME} --input-blocks=$SOURCE
EOF
