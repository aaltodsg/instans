...
(defun instans-add-rules (rete-iri rules-iri &key output-directory report-function report-function-arguments)
...
(defun instans-add-triples-from-url (rete-iri triples-url graph-iri)
  (let ((network (instans-get-rete rete-iri)))
    (cond ((sparql-error-p network) network)
	  (t
	   (let* ((input-name (rdf-iri-string triples-url))
		  (data (drakma:http-request input-name))
		  (string (cond ((stringp data) data) (t (coerce (mapcar #'code-char (coerce data 'list)) 'string)))))
	     (inform "~S" string)
	     (with-input-from-string (triples-stream string)
	       (let* ((triples-lexer (make-turtle-lexer triples-stream))
		      (triples-parser (make-turtle-parser :triples-callback #'(lambda (triples)
										(inform "~%Event callback: ~D triples~%" (length triples))
										(loop for tr in triples do (inform " ~S~%" tr))
										(process-triple-input network triples '(:add :execute) :graph graph-iri)))))
		 (warn "~%Processing triples:~%")
		 (time (funcall triples-parser triples-lexer))
		 rete-iri)))))))
