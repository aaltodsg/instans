...
(defun call-succ-nodes (func node token stack)
  (cond ((null stack)
	 ...
	 (let* ((instans (node-instans node))
		(reportp (member :call-succ-nodes (rule-instance-queue-report-p (instans-rule-instance-queue instans))))
...
(defun rule-instance-queue-add (queue node token)
  (let* ((instance (make-instance 'rule-instance :node node :token token))
	...
	(report-rule-op "Added" node token))
...
(defun rule-instance-queue-remove (queue node token)
  (loop for prev = nil then list
	...
	do (progn
	     ...
	     (if (member :queue (rule-instance-queue-report-p (instans-rule-instance-queue (node-instans node))))
		 (report-rule-op "Removed existing" node token))
	     (return :found))
	finally (progn
		  (if (member :queue (rule-instance-queue-report-p (instans-rule-instance-queue (node-instans node))))
		      (report-rule-op "Tried to remove non-existing" node token))
		  (return :not-found))))
...
(defun rule-instance-queue-execute-instance (queue instance)
  (let ((node (rule-instance-node instance))
    ...
    (cond ((typep node 'select-node)
	   (if (member :select rule-types) (report-rule-op "Executing" node token))
	   (incf (rule-instance-queue-select-count queue)))
	  ((typep node 'ask-node)
	   (if (member :ask rule-types) (report-rule-op "Executing" node token))
	   (incf (rule-instance-queue-ask-count queue)))
	  ((typep node 'describe-node)
	   (if (member :describe rule-types) (report-rule-op "Executing" node token))
	   (incf (rule-instance-queue-describe-count queue)))
	  ((typep node 'modify-node)
	   (if (member :modify rule-types) (report-rule-op "Executing" node token))
	   (incf (rule-instance-queue-modify-count queue)))
	  ((typep node 'construct-node)
	   (if (member :construct rule-types) (report-rule-op "Executing" node token))
