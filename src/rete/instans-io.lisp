	...
	(xml-emitter:with-tag ("head")
	  (when (slot-boundp this 'variables)
	    (loop for variable in (sparql-query-results-variables this)
		  do (xml-emitter:with-simple-tag ("variable" `(("name" ,(format nil "~(~A~)" (subseq (instans-var-pretty-name variable) 1)))))))
		       ...
		       do (xml-emitter:with-tag ("result")
			    (loop for variable in (sparql-query-results-variables this)
				  ...
				  do (let ((value (sparql-binding-value binding)))
					 ...
					 (xml-emitter:with-tag ("binding" `(("name" ,(format nil "~(~A~)" (subseq (instans-var-pretty-name variable) 1)))))
					...
					;(inform "writing blank ~A~%" (instans-var-pretty-name value))
						  (xml-emitter:with-simple-tag ("bnode") (xml-emitter:xml-as-is (instans-var-pretty-name value))))
...
[]      rdf:type    rs:ResultSet ;")
    (cond ((slot-boundp this 'variables)
	   (format stream "        ~{rs:resultVariable \"~A\" ;~^~%        ~}"
		   (mapcar #'(lambda (v) (subseq (instans-var-pretty-name v) 1)) (sparql-query-results-variables this)))
	     ...
	     (format stream "        ~{rs:solution [ ~{ rs:binding ~{ [ rs:variable \"~A\"; rs:value    ~A ]~}~^;~%                      ~}
		     ...
		     (mapcar #'(lambda (solution) (mapcan #'(lambda (b)
							      (and (not (sparql-unbound-p (sparql-binding-value b)))
								   (list (list (subseq (instans-var-pretty-name (sparql-binding-variable b)) 1)
  ...
  (:method ((this instans-csv-writer) variables)
    (unless (instans-writer-append-p this)
      (write-csv-headers (instans-csv-writer-csv-output this)
			 (mapcar #'(lambda (var)
				     (format nil "~(~A~)" (subseq (instans-var-pretty-name var) 1)))
				 variables))))
