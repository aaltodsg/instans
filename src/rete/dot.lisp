...
(defgeneric dot-node-pretty-name-format-string (node)
  ...
  (:method ((this bind-node)) (dot-default-pretty-name-format (format nil "B[~A]" (instans-var-pretty-name (bind-variable this)))))
...
(defun dot-pretty-triple-pattern (node)
  (format nil "~{~a~^ ~}" (mapcar #'(lambda (x) (cond ((sparql-var-p x)
						       (string-downcase (string (instans-var-pretty-name (reverse-resolve-binding (node-instans node) x)))))
...
;	  do (inform "var = ~S" var)
	  unless (null var)
	  collect (instans-var-pretty-name (car (rassoc var alist))))))
	...
	(let* ((rules (read-from-url-or-file rules-file))
	       (bindings (with-output-to-string (str)
			   ...
			   (loop for (from . to) in (instans-bindings instans)
				 do (format str "~%  ~A -> ~A" (instans-var-pretty-name from) (instans-var-pretty-name to)))))
