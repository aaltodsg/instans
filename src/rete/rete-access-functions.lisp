...
;;   (let ((seen nil))
;;     (labels ((revresol (node vars) (reverse-resolve-bindings (node-instans node) vars))
;; 	     (visit (node)
...
;; 		     (inform "(node-all-vars-in ~S) = ~S" node (revresol node (node-all-vars-in node)))
;; 		     (setf (node-visible-vars-in node) (reduce #'sparql-var-list-union (mapcar #'node-visible-vars-out precs) :initial-value nil))
;; 		     (inform "(node-visible-vars-in ~S) = ~S" node (revresol node (node-visible-vars-in node)))
...
;; 						 (t nil)))
;; 		     (inform "(node-use ~S) = ~S" node (revresol node (node-use node)))
;; 		     (setf (node-def-prec node) (reduce #'sparql-var-list-union (mapcar #'node-def-preceq precs) :initial-value nil))
;; 		     (inform "(node-def-prec ~S) = ~S" node (revresol node (node-def-prec node)))
...
;; 						 (t nil)))
;; 		     (inform "(node-def ~S) = ~S" node (revresol node (node-def node)))
...
;; 		     (inform "(node-all-vars-out ~S) = ~S" node (revresol node (node-all-vars-out node)))
;; 		     (inform "(node-kill ~S) = ~S" node (revresol node (node-kill node)))
...
;; 		     (inform "(node-visible-vars-out ~S) = ~S" node (revresol node (node-visible-vars-out node)))
...
;; 		       (inform "(join-beta-minus-alpha-vars ~S) = ~S" node (revresol node (join-beta-minus-alpha-vars node)))
...
;; 		       (inform "(join-alpha-minus-beta-vars ~S) = ~S" node (revresol node (join-alpha-minus-beta-vars node)))
;; 		       )
;; 		     (describe node)
;; 		     ))))
