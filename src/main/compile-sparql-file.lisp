		      ...
		      (let* ((key-lambda `(lambda ,(sparql-var-lisp-names (aggregate-join-key-vars node))
									...
									(setf (group-aggregates ,group-arg)
									      (list ,@(loop for aggr-expr in (aggregate-join-aggr-exprs node)
											 collect (cond ((eq (first aggr-expr) (find-sparql-op "aggregate_sample"))
													...
													`(make-instance 'aggregate-group-concat :separator (sparql-expr-to-lisp (third aggr-expr)))))))))))
						   ,@(loop for aggr-expr in (aggregate-join-aggr-exprs node)
							collect `(let ((,aggr-temp (pop ,aggrs-temp)))
								   (inform "add-value ~A to ~A" ',(sparql-expr-to-lisp (second aggr-expr)) ,aggr-temp)
								   (aggregate-add-value ,aggr-temp ,(sparql-expr-to-lisp (second aggr-expr)))))
						   (inform "end aggr-add~%"))))
			     (aggr-remove-lambda `(lambda (,instans-arg ,group-arg ,@(sparql-var-lisp-names (aggregate-join-aggr-vars node)))
						    ...
						    (let ((,aggrs-temp (group-aggregates ,group-arg)))
						      ,@(loop for aggr-expr in (aggregate-join-aggr-exprs node)
							   collect `(let ((,aggr-temp (pop ,aggrs-temp)))
								      (inform "remove-value ~A from ~A" ',(sparql-expr-to-lisp (second aggr-expr)) ,aggr-temp)
								      (aggregate-remove-value ,aggr-temp ,(sparql-expr-to-lisp (second aggr-expr)))))))))
