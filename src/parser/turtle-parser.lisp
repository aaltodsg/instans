...
(defun make-turtle-parser (&key (triples-callback nil))
  (let* ((triples (list nil))
    ...
    (labels ((clear-result ()
	     ...
	     (expand-collection (col)
	       (cond ((null col) *rdf-nil*)
			...
			subj))))
	     (set-prefix (prefix expansion) (rebind-prefix lexer prefix expansion))
	     (set-base (b) (set-lexer-base lexer b) (values))
	     (pname2iri (prefix &optional suffix) (pname-to-iri lexer prefix suffix)))
	...
	((turtleDoc		  ::= (:REP0 statement) :RESULT (values))
	 ...
	 (base		  ::= (@BASE-TERMINAL IRIREF-TERMINAL .-TERMINAL :RESULT (set-base $1)))
	 ...
	 (triples		  ::= (:OR (subject predicateObjectList :RESULT (emit-subj-pred-obj-list $0 $1))
				  ...
				  :RESULT (progn (when triples-callback (funcall triples-callback (result))) (values)))
	 ...
	 (BooleanLiteral	  ::= (:OR (TRUE-TERMINAL :RESULT t)
					   (FALSE-TERMINAL :RESULT nil)))
	 ...
	 (PrefixedName          ::= (:OR (PNAME_LN-TERMINAL :RESULT (pname2iri $0)) (PNAME_NS-TERMINAL :RESULT (pname2iri (first $0) (second $0)))))
	 (BlankNode		  ::= (:OR (BLANK_NODE_LABEL-TERMINAL :RESULT (make-rdf-blank-node $0)) (ANON-TERMINAL :RESULT (generate-rdf-blank-node)))))
	#'(lambda (lexer &rest keys &key &allow-other-keys)
	    (clear-result)
