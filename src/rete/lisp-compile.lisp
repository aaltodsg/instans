...
;	do (inform "compiling ~A" node)
	do (cond ((filter-node-p node)
		  (let* ((filter-test (filter-test node))
			 (filter-test-lambda `(lambda ,(sparql-var-lisp-names (node-use node)) ,@(translate-sparql-expr (node-instans node) (sparql-expr-to-lisp filter-test)))))
;		    (inform "filter-test-lambda = ~S" filter-test-lambda)
		 ...
		 ((bind-node-p node)
		  (let* ((bind-form (bind-form node))
			 (bind-form-lambda `(lambda ,(sparql-var-lisp-names (node-use node)) ,@(translate-sparql-expr (node-instans node) (sparql-expr-to-lisp bind-form)))))
