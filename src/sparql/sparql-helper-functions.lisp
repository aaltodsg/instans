...
(defun sparql-value-to-sexp (x &key instans)
  ...
  (labels ((clean-decimal-string (str)
	     (let ((di (position #\d str :test #'char=)))
	       (cond ((null di) str)
		     ((and (= di (- (length str) 2)) (char= #\0 (char str (1- (length str)))))
		      (subseq str 0 di))
		     (t (setf (char str di) #\e)
			str)))))
    (cond ((typep x 'xsd-string-value) `(parse-xsd-string ,x))
	  ...
	  ((typep x 'xsd-datetime-value) `(parse-xsd-datetime ,(datetime-canonic-string x)))
	  ((rdf-literal-p x)
	   (cond ((rdf-literal-type x)
		  `(create-rdf-literal-with-type ,(rdf-literal-string x) (parse-iri ,(iri-to-string (rdf-literal-type x) nil t))))
		 ((rdf-literal-lang x)
		  `(create-rdf-literal-with-lang ,(rdf-literal-string x) ,(rdf-literal-lang x)))
		 (t
		  `(parse-xsd-string ,(rdf-literal-string x)))))
	  ((rdf-iri-p x) `(parse-iri ,(iri-to-string x nil t)))
