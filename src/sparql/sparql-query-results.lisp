...
(define-class sparql-old-test ()
  ((test-set :accessor sparql-old-test-test-set :initarg :test-set)
   (type :accessor sparql-old-test-type :initarg :type)
   (suite :accessor sparql-old-test-suite :initarg :suite)
   (collection :accessor sparql-old-test-collection :initarg :collection)
   (name :accessor sparql-old-test-name :initarg :name)
   (queryfile :accessor sparql-old-test-queryfile :initarg :queryfile)
   (datafile :accessor sparql-old-test-datafile :initarg :datafile)
   (graphdatafiles :accessor sparql-old-test-graphdatafiles :initarg :graphdatafiles)
   (resultfile :accessor sparql-old-test-resultfile :initarg :resultfile)
   (resultgraphs :accessor sparql-old-test-resultgraphs :initarg :resultgraphs)
   (error-message :accessor sparql-old-test-error-message :initform nil)
   (completed :accessor sparql-old-test-completed :initform nil)
   (implemented :accessor sparql-old-test-implemented :initform t)
   (negative :accessor sparql-old-test-negative :initarg :negative)
   (parse :accessor sparql-old-test-parse :initform nil)
   (translate :accessor sparql-old-test-translate :initform nil)
   (initialization :accessor sparql-old-test-initialization :initform nil)
   (run :accessor sparql-old-test-run :initform nil)
   (comparable :accessor sparql-old-test-comparable :initform nil)
   (compare :accessor sparql-old-test-compare :initform nil)
   (pass :accessor sparql-old-test-pass :initform nil)))

(defgeneric sparql-old-test-successful-p (sparql-old-test)
  (:method ((this sparql-old-test))
    (cond ((not (sparql-old-test-completed this)) (error* "Test ~A not completed yet!" (sparql-old-test-name this)))
	  ((not (sparql-old-test-implemented this)) nil)
	  ((sparql-old-test-negative this) (not (sparql-old-test-parse this)))
	  ((not (sparql-old-test-parse this)) nil)
	  ((not (sparql-old-test-translate this)) nil)
	  ((not (sparql-old-test-initialization this)) nil)
	  ((not (sparql-old-test-run this)) nil)
	  ((not (sparql-old-test-compare this)) (not (sparql-old-test-comparable this)))
	  (t t))))

(define-class sparql-old-test-set ()
  ((entries :accessor sparql-old-test-set-entries :initarg :entries :initform nil)
   (fields :accessor sparql-old-test-set-fields
	   :allocation :class
	   :initform '(suite collection name queryfile datafile implemented negative parse translate run comparable compare pass))))

(defun sparql-old-test-fields (test) (sparql-old-test-set-fields (sparql-old-test-test-set test)))

(defgeneric add-sparql-old-test (sparql-old-test-set &rest keys &key type negative suite collection name queryfile datafile graphdatafile resultfile resultgraphfile resultgraphlabel)
  (:method ((this sparql-old-test-set) &rest keys &key type negative suite collection name queryfile datafile graphdatafile resultfile resultgraphfile resultgraphlabel)
    (declare (ignorable keys))
;    (inform "enter add-sparql-old-test ~A" keys)
    (flet ((string=* (x y)
	     (cond ((null x) (null y))
		   ((null y) nil)
		   (t (string= x y)))))
      (let ((matching (find-if #'(lambda (test)
				   (and (string=* (sparql-old-test-name test) name)
					(string=* (sparql-old-test-suite test) suite)
					(string=* (sparql-old-test-collection test) collection)
					(string=* (sparql-old-test-queryfile test) queryfile)))
			       (sparql-old-test-set-entries this))))
;	(inform "matching = ~A" matching)
	(cond ((null matching)
	       (push-to-end (setf matching (make-instance 'sparql-old-test
							  :test-set this
			    ...
			    (sparql-old-test-set-entries this)))
	      (t
	       (setf (sparql-old-test-datafile matching) (or (sparql-old-test-datafile matching) datafile))
	       (setf (sparql-old-test-resultfile matching) (or (sparql-old-test-resultfile matching) resultfile))
	       (when graphdatafile
		 (push-to-end-new graphdatafile (sparql-old-test-graphdatafiles matching) :test #'equal))
	       (when resultgraphfile
		 (push-to-end-new (list resultgraphfile resultgraphlabel) (sparql-old-test-resultgraphs matching)
				  :test #'(lambda (a b)
					    (and (equal (first a) (first b)) (equal (second a) (second b))))))
;	       (describe matching)
	       ))
	(setf (sparql-old-test-comparable matching) (not (null (or (sparql-old-test-resultfile matching) (sparql-old-test-resultgraphs matching)))))))
					;    (inform "exit add-sparql-old-test ~A" keys)
    ))

(defgeneric print-sparql-old-test-set (sparql-old-test-set &key output output-type)
  (:method ((this sparql-old-test-set) &key output (output-type :csv))
    (let (stream)
	     ...
	     (setf stream
		   (cond ((typep output 'stream) stream)
			 (t ;(inform "open-file ~A" output)
			    (open-file output :direction :output :if-exists :supersede :fmt "print-sparql-old-test-set: open ~{~A~^ ~}"))))
	     (print-sparql-old-test-headers this :stream stream :output-type output-type)
	     (loop for test-number from 1
		   for test in (sparql-old-test-set-entries this)
		   do (print-sparql-old-test test :stream stream :output-type output-type :test-number test-number))
	     (print-sparql-old-test-trailers this :stream stream :output-type output-type))
	(unless (typep output 'stream)
;	  (inform "closing ~A" stream)
	  (close-stream stream "print-sparql-old-test-set: close ~A"))))))

(defgeneric print-sparql-old-test-headers (sparql-old-test-set &key stream output-type)
  (:method ((this sparql-old-test-set) &key stream (output-type :csv))
    (case output-type
      (:txt)
      (:lisp (error* "Not implemented yet"))
      (:csv (format stream "~{~A~^,~}" (sparql-old-test-set-fields this)))
      (:html ;(inform "(sparql-old-test-set-fields this) = ~A, stream = ~A" (sparql-old-test-set-fields this) stream)
...
<tbody>~%" (mapcar #'(lambda (f) (list f f)) (sparql-old-test-set-fields this))))
      (t (error* "Unknown output-type ~A" output-type)))))

(defgeneric print-sparql-old-test-trailers (sparql-old-test-set &key stream output-type)
  (:method ((this sparql-old-test-set) &key stream (output-type :csv))
    (case output-type
      (:txt)
      (:lisp)
...
(defgeneric print-sparql-old-test (sparql-old-test &key stream output-type test-number)
  (:method ((this sparql-old-test) &key stream (output-type :csv) test-number)
    (flet ((sparql-value-to-string* (v) (if (stringp v) v (sparql-value-to-string v))))
      (case output-type
	(:txt (format stream "~&sparql-old-test:~%~{  ~{~(~A~) = ~A~}~^~%~}~%" (mapcar #'(lambda (field) (list field (slot-value this field))) (sparql-old-test-fields this))))
	(:lisp (error* "Not implemented yet"))
	(:csv (format stream "~&~{~A~^,~}~%" (mapcar #'(lambda (f) (sparql-value-to-string* (slot-value this f))) (sparql-old-test-fields this))))
	(:html (format stream "~&<tr class=\"~(~A~)-~(~A~)\" id=\"~(~A~)-~(~A~)_~(~A~)\">~%<td class=\"test-number\">~A</td>~%~{~{  <td class=\"~(~A~)\">~(~A~)</td>~}</td>~^~%~}~%</tr>~%"
		       (sparql-old-test-suite this) (sparql-old-test-collection this)(sparql-old-test-suite this) (sparql-old-test-collection this) (sparql-old-test-name this) (if test-number test-number "") (mapcar #'(lambda (f) (list f (slot-value this f))) (sparql-old-test-fields this))))
	(t (error* "Unknown output-type ~A" output-type))))))
...
(defgeneric expand-sparql-old-test-path (sparql-old-test pathname)
  (:method ((this sparql-old-test) pathname)
    (and pathname (format nil "~Atests/~A/~A/~A" (find-instans-root-directory) (sparql-old-test-suite this) (sparql-old-test-collection this) pathname))))

(defgeneric run-one-sparql-old-test (sparql-old-test &key output-dir-name log-file print-queryfile print-datafile print-resultfile)
  (:method ((this sparql-old-test) &key (output-dir-name "cmpoutput") log-file print-queryfile print-datafile (print-resultfile nil))
    (unless (char= (char output-dir-name (1- (length output-dir-name))) #\/)
      (setf output-dir-name (format nil "~A/" output-dir-name)))
    (let (log-stream)
      (unwind-protect
	   (progn
	     (setf (sparql-old-test-completed this) nil)
	     (setf log-stream (if log-file
				  (open-file log-file :direction :output :if-exists :append :if-does-not-exist :create :fmt "run-one-sparql-old-test: open ~{~A~^ ~}")
				  *error-output*))
	     (format log-stream "~&---------------~%")
	     (format log-stream "~&~A-~A-~A~%" (sparql-old-test-suite this) (sparql-old-test-collection this) (sparql-old-test-name this))
;	     (print-sparql-old-test this :stream log-stream :output-type :txt)
	     (format log-stream "~&---------------~%")
	     (let* ((queryfile (expand-sparql-old-test-path this (sparql-old-test-queryfile this)))
		    (output-directory (expand-sparql-old-test-path this output-dir-name))
		    (datafile (expand-sparql-old-test-path this (sparql-old-test-datafile this)))
		    (graph-datafiles (mapcar #'(lambda (file) (expand-sparql-old-test-path this file)) (sparql-old-test-graphdatafiles this)))
		    (resultfile (expand-sparql-old-test-path this (sparql-old-test-resultfile this)))
...
;		    (resultgraphs (mapcar #'(lambda (x) (list (expand-sparql-old-test-path this (first x)) (second x))) (sparql-old-test-resultgraphs this)))
		 ...
		 (inform "Query ~A:~%~A~%" queryfile (file-contents-to-string queryfile)))
	       ...
	       (inform "Trying to add rules in ~A" (sparql-old-test-name this))
	       (instans-add-rules instans queryfile)
					;		   (untrace)
	       (setf (sparql-old-test-parse this) (instans-has-status instans (intern-instans "INSTANS-RULE-PARSING-SUCCEEDED")))
	       (inform "Test ~A parse = ~A" (sparql-old-test-name this) (sparql-old-test-parse this))
	       (setf (sparql-old-test-translate this) (instans-has-status instans (intern-instans "INSTANS-RULE-TRANSLATION-SUCCEEDED")))
	       (setf (sparql-old-test-initialization this) (instans-has-status instans (intern-instans "INSTANS-RULE-INITIALIZATION-SUCCEEDED")))
	       (let* ((runp (and (sparql-old-test-parse this) (sparql-old-test-translate this)))
		 ...
		 (when runp
		   (handler-case
		     ...
		     (t (e)
		       (inform "Adding datafile(s) failed in test ~A: ~A" (sparql-old-test-name this) e)
		       (setf runp nil))))
		   ;;; Run the tests, check for status
		 (when runp
		   (instans-run instans)
		   (instans-close-open-streams instans)
		   (setf (sparql-old-test-run this) (instans-has-status instans 'instans-rule-running-succeeded))
		   (setf (sparql-old-test-implemented this) (not (instans-has-status instans 'instans-feature-not-implemented-yet))))
		 (when (and (sparql-old-test-run this) (sparql-old-test-comparable this))
		   (cond (select-rules-p
		   ...
		   (setf (sparql-old-test-compare this) compare-result)
	     ...
	     (setf (sparql-old-test-completed this) t)
	     (setf (sparql-old-test-pass this) (sparql-old-test-successful-p this))
	     (print-sparql-old-test this :output-type :csv :stream log-stream)
	     )
	(when log-file (close-stream log-stream "run-sparql-old-test: closing ~A"))))))

(defgeneric run-sparql-old-test-set (sparql-old-test-set &key output-dir-name names-of-tests-to-run names-of-tests-to-avoid name-of-first-test-to-start-at log-file)
  (:method ((this sparql-old-test-set) &key (output-dir-name "cmpoutput") names-of-tests-to-run
    ...
    (let ((entries (sparql-old-test-set-entries this)))
      (when name-of-first-test-to-start-at
	(loop for test = (first entries)
	      while (not (search name-of-first-test-to-start-at (sparql-old-test-name test) :test #'equalp))
	      do (pop entries)))
      (loop for test in entries
	    for test-name = (sparql-old-test-name test)
	    ...
	    do (run-one-sparql-old-test test :output-dir-name output-dir-name :log-file log-file)))))

(defvar *sparql-old-test-set* nil)

(defun rst (&optional suite collection name)
  (loop for test in (sparql-old-test-set-entries (or *sparql-old-test-set* (parse-tests)))
	when (and (or (null suite) (equal suite (sparql-old-test-suite test)))
		  (or (null collection) (equal collection (sparql-old-test-collection test)))
		  (or (null name) (equal name (sparql-old-test-name test))))
	do (progn
	     (inform "Running ~A-~A-~A" (sparql-old-test-suite test) (sparql-old-test-collection test) (sparql-old-test-name test))
	     (run-one-sparql-old-test test :log-file nil :print-queryfile t :print-datafile t :print-resultfile t)
	     (print-sparql-old-test test :stream *error-output* :output-type :txt))))

(defun parse-tests (&key (rules-file (format nil "~A/tests/input/gettestfiles.rq" (find-instans-root-directory)))
		      (tests-csv-file (format nil "~A/tests/sparql-old-test-set/sparql-old-test-set.csv" (find-instans-root-directory))))
  (tests-from-manifests :rules-file rules-file :select-output-name tests-csv-file)
  (setf *sparql-old-test-set* (parse-csv-tests tests-csv-file)))
...
(defun parse-csv-tests (csv-file &key (skip-lines 1))
  (let ((tests (make-instance 'sparql-old-test-set)))
...
(defun categorize-data-file (filename)
  (let ((type (and (file-type filename) (intern-keyword (string-upcase (file-type filename))))))
    type))

(defun categorize-result-file (filename)
  (let ((type (and (file-type filename) (intern-keyword (string-upcase (file-type filename))))))
...
(defun categorize-tests (&optional (key-length 4))
  (multiple-value-bind (test-alist headers) (csv-file-to-alist "/Users/enu/instans/tests/sparql-old-test-set/sparql-old-test-set.csv" :key-fields '("type" "suite" "collection" "name") :data-field-merge #'simple-field-merge)
...
;			 (inform "old-key=~A, old-data=~A~%new-key=~A, new-data=~A" key data category-key category-data)
	      ...
	      (loop for test in abstract-test-alist do (categorize-test test))
	      (values category-alist headers))))))))

(defun show-categorize-tests (&optional (key-length 3))
  (multiple-value-bind (category-alist headers) (categorize-tests key-length)
;    (inform "Headers: ~{~A~^ ~}" headers)
    (let ((*print-right-margin* 250))
      ;; (loop for test in category-alist
      ;; 	    do (inform "~A ~{~A~^-~}:~{~%  ~S~}" (caar test) (cdar test) (rest test)))
      (values category-alist headers))))
...
(defun process-sparql-old-test-suite (&key (rules-file (format nil "~A/tests/input/gettestfiles.rq" (find-instans-root-directory)))
				    (tests-csv-file (format nil "~A/tests/sparql-old-test-set/sparql-old-test-set.csv" (find-instans-root-directory)))
				    (results-type :html)
				    (results-file (format nil "~A/tests/sparql-old-test-set/sparql-old-test-set-results.~(~A~)" (find-instans-root-directory) results-type))
				    (log-file (format nil "~A/tests/sparql-old-test-set/sparql-old-test-set-log" (find-instans-root-directory))))
  (let ((sparql-old-test-set (or *sparql-old-test-set* (parse-tests :rules-file rules-file :tests-csv-file tests-csv-file)))
	(*oink* t))
    (declare (special *oink*))
    (run-sparql-old-test-set sparql-old-test-set :log-file log-file)
    (print-sparql-old-test-set sparql-old-test-set :output-type results-type :output results-file)))

(defun psts ()
  (process-sparql-old-test-suite :results-file nil :results-type :txt :log-file nil))
