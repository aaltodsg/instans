...
(defgeneric store-get-token (memory token)
  (:method ((this memory) token)
    (let* ((key (second (car token)))
      ...
      (checkit (or (null stored-token) (equal token stored-token)) "token ~A and stored-token ~A not equal" token stored-token)
      stored-token))
  (:method ((this filter-memory) token)
    (let* ((key (second (car token)))
	   (stored-token (gethash key (memory-store this))))
      (checkit (numberp key) "key ~S not a number" key)
      (checkit (or (null stored-token) (equal token (cddr stored-token))) "token ~A and stored-token ~A not equal" token (cddr stored-token))
      stored-token))
  (:method ((this existence-start-node) token)
    (let* ((key (second (car token)))
	   (stored-token (gethash key (memory-store this))))
      (checkit (numberp key) "key ~S not a number" key)
      (checkit (or (null stored-token) (equal token (cdddr stored-token))) "token ~A and stored-token ~A not equal" token (cddr stored-token))
      stored-token)))
