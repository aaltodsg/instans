...
(defun make-new-nonterminal (grammar prefix-nonterminal)
  (let ((nonterminals (grammar-nonterminals grammar))
    ...
    (loop for new-nonterminal = (fmt-intern "~A'" prefix-nonterminal)
...
(defun generate-ll1-table (grammar)
  (let* ((terminals (append (grammar-terminals grammar) (list '$)))
    ...
    (flet ((make-arglist (n) (loop for i from 0 below n collect (fmt-intern "$~D" i))))
      (loop for i from 0
	    for p in productions
	    ...
	    do (progn
		 ...
		 (let* ((with-args-p (and (consp result) (eq (car result) 'with-args)))
			...
			(result-func-name (and (or result default-result) (fmt-intern "~A-~D" (production-lhs p) (production-number p))))
    ...
    (let* ((*parser* parser)
      ...
      (labels ((nonterminalp (grammar-symbol-value) (< -1 grammar-symbol-value nonterminal-count))
		 ...
		 (cond ((ll-parser-saved-input-token parser)
			(when (debugp subscribe :token) (inform "using saved token ~S" (ll-parser-saved-input-token parser)))
			...
			(let ((input-token (get-input-token lexer)))
			  (when (debugp subscribe :token) (inform "~%lexer returns ~S" input-token))
			  (cond ((error-input-token-p input-token)
				...
				((eof-input-token-p input-token)
				 ...
				 (let ((token (make-input-token :type '$ :index (grammar-symbol-value parser '$))))
				   (maybe-store-token token)
				   token))
				(t
				 (let ((token-index (grammar-symbol-value parser (input-token-type input-token))))
				   (unless token-index (error* "Could not find the index for type of token ~A" (input-token-type input-token)))
				   (setf (input-token-index input-token) token-index)
				   (maybe-store-token input-token)
				   input-token)))))))
	       (maybe-store-token (token)
		 (when (ll-parser-store-tokens-p parser)
		   (push token (ll-parser-stored-token-stack parser)))
		 token)
	       (print-snapshot (input-token)
		 (inform "~%Round ~D~%Parser ~S~%Input ~S~%Input-token = ~A~%Stack = "
			 (ll-parser-round parser)
			 parser (lexer-input-stream lexer)
			 (if (input-token-p input-token)
			     (format nil "(input-token ~S (~D) ~S at ~S)" (input-token-type input-token) (input-token-index input-token) (input-token-value input-token) (input-token-position input-token))
			     input-token))
...
;		   (update-parser-statistics parser)
		   ...
		   (let ((top (first (ll-parser-stack parser)))
			 (symbol (input-token-index input-token)))
			    ...
			    (let ((table-value (parse-table-entry top symbol)))
			      (cond ((eq table-value 'error)
				     ...
				     (ll-parser-failure "No legal action for nonterminal ~A and input symbol (~A ~S))"
							(second (number-to-symbol-or-production top parser)) (input-token-type input-token)
			   ...
			   (t ; (productionp top)
				...
				(when result-func
				  (let ((args nil))
				    ...
				    ;;   (labels ((ppx (x) (cond ((listp x) (mapcar #'ppx x))
				    ;; 			      ((rdf-term-p x) (rdf-term-as-string x))
				    ;; 			      (t (format nil "~S" x)))))
					...
					(print-snapshot input-token)
					(setf (ll-parser-print-snapshot-p parser) nil))
				      (cond ((not threwp)
					     ...
					     (when (eq (ll-parser-state parser) :yield)
					       (setf (ll-parser-saved-input-token parser) input-token) ; Save input-token for next call of parser
					       (when (debugp subscribe :parse-operations)
						 (inform "!!! Parser yields~%Values = ~S~%Saved token = ~S" values (ll-parser-saved-input-token parser)))
 					       (throw 'parsed parser)))
					    (t
					     (unless (member (ll-parser-state parser) '(:succeeded :failed))
					       (error* "Parser throws, state is ~A" (ll-parser-state parser)))
