			...
			(setf (bind-form-func node) (compile nil bind-lambda))))
		     ((aggregate-join-node-p node)
		      (let* ((key-lambda `(lambda ,(sparql-var-lisp-names (aggregate-join-key-vars node))
			     ...
			     (aggr-lambda `(lambda (,instans-arg ,group-arg ,@(sparql-var-lisp-names (aggregate-join-aggr-vars node)))
					     (let ((,aggrs-temp (group-aggregates ,group-arg)))
					       (cond ((null ,aggrs-temp)
						      ;; ,@(create-aggregates (aggregate-join-aggr-exprs node))
						      (setf (group-aggregates ,group-arg)
							    (list ,@(loop for aggr-expr in (aggregate-join-aggr-exprs node)
								       collect (cond ((eq (first aggr-expr) (find-sparql-op "aggregate_sample"))
										      `(make-instance 'aggregate-sample :initial-value ,(sparql-expr-to-lisp (second aggr-expr))))
										      ((eq (first aggr-expr) (find-sparql-op "aggregate_count"))
										      `(make-instance 'aggregate-count :initial-value ,(sparql-expr-to-lisp (second aggr-expr))))
										      ((eq (first aggr-expr) (find-sparql-op "aggregate_sum"))
										      `(make-instance 'aggregate-sum :initial-value ,(sparql-expr-to-lisp (second aggr-expr))))
										      ((eq (first aggr-expr) (find-sparql-op "aggregate_avg"))
										      `(make-instance 'aggregate-avg :initial-value ,(sparql-expr-to-lisp (second aggr-expr))))
										      ((eq (first aggr-expr) (find-sparql-op "aggregate_min"))
										      `(make-instance 'aggregate-min :initial-value ,(sparql-expr-to-lisp (second aggr-expr))))
										      ((eq (first aggr-expr) (find-sparql-op "aggregate_max"))
										      `(make-instance 'aggregate-max :initial-value ,(sparql-expr-to-lisp (second aggr-expr))))
										      ((eq (first aggr-expr) (find-sparql-op "aggregate_group_concat"))
										      `(make-instance 'aggregate-group-concat :initial-value ,(sparql-expr-to-lisp (second aggr-expr))
												      :separator (sparql-expr-to-lisp (third aggr-expr)))))))))
						     (t
						      ;; ,@(update-aggregates (aggregate-join-aggr-exprs node))
						      ,@(loop for aggr-expr in (aggregate-join-aggr-exprs node)
							   collect `(let ((,aggr-temp (pop ,aggrs-temp)))
								      (aggregate-update ,aggr-temp ,(sparql-expr-to-lisp (second aggr-expr)))))))))))
			(inform "(aggregate-join-key-lambda node ~A) = ~%~A" node key-lambda)
			(inform "(aggregate-join-aggr-lambda node ~A) = ~%~A" node aggr-lambda)
