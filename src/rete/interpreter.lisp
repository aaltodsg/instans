...
(defun rule-instance-queue-execute-instance (queue rule-instance)
  (let ((node (rule-instance-node rule-instance))
    ...
    (cond ((typep node 'select-node)
	   (if (member :select rule-types) (report-rule-op queue "Executing" rule-instance))
	   (incf (rule-instance-queue-select-count queue)))
	  ((typep node 'ask-node)
	   (if (member :ask rule-types) (report-rule-op queue "Executing" rule-instance))
	   (incf (rule-instance-queue-ask-count queue)))
	  ((typep node 'describe-node)
	   (if (member :describe rule-types) (report-rule-op queue "Executing" rule-instance))
	   (incf (rule-instance-queue-describe-count queue)))
	  ((typep node 'modify-node)
	   (if (member :modify rule-types) (report-rule-op queue "Executing" rule-instance))
	   (incf (rule-instance-queue-modify-count queue)))
	  ((typep node 'construct-node)
	   (if (member :construct rule-types) (report-rule-op queue "Executing" rule-instance))
