...
;		   (inform "~s precs = ~S" node precs)
		   (loop for prec in precs do (visit prec))
		   (setf (node-all-vars-in node) (reduce #'list-union (mapcar #'node-all-vars-out precs) :initial-value nil))
		   (setf (node-visible-vars-in node) (reduce #'list-union (mapcar #'node-visible-vars-out precs) :initial-value nil))
		   (setf (node-use node) (cond ((typep node 'join-node)
						(list-intersection (node-def-preceq (join-alpha node)) (node-def-preceq (join-beta node)) :test #'sparql-var-equal))
						...
						(list-union (unless (eq (solution-modifiers-project node) '*)
						...
						(list-union (modify-delete-parameters node) (modify-insert-parameters node) :test #'sparql-var-equal))
					       (t nil)))
		   (setf (node-def-prec node) (reduce #'list-union (mapcar #'node-def-preceq precs) :initial-value nil))
		   ...
		   (setf (node-visible-vars-out node) (list-difference (list-union (node-visible-vars-in node) (node-def node) :test #'sparql-var-equal)
								       (node-kill node) :test #'sparql-var-equal))
		   (when (join-node-p node)
		     (setf (join-beta-minus-alpha-vars node) (list-difference (node-all-vars-out (join-beta node)) (node-all-vars-out (join-alpha node)) :test #'sparql-var-equal))
		     (setf (join-alpha-minus-beta-vars node) (list-difference (node-all-vars-out (join-alpha node)) (node-all-vars-out (join-beta node)) :test #'sparql-var-equal)))
;		   (describe node)
		   ))))
