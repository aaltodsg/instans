...
(defun avl-get-range (tree &key lower-bound lower-bound-inclusive-p upper-bound upper-bound-inclusive-p key-compare (node-value-getter #'(lambda (n) (avl-value n))))
  (let* ((result (list nil))
	 (tail result))
    (labels ((add-to-result (node)
	     ...
	     (visit (node)
	       (when (and (avl-left node) (or (null lower-bound) (plusp (funcall key-compare (avl-key node) lower-bound))))
		 (visit (avl-left node)))
	       (when (inclusion-test node)
		 (add-to-result node))
	       (when (and (avl-right node) (or (null upper-bound) (plusp (funcall key-compare upper-bound (avl-key node)))))
