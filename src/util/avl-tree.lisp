...
;    (inform "~%key-compare = ~S~%" key-compare)
    (labels ((add-to-result (node)
	     ...
	     (visit (node)
;	       (inform "node ~S, node-key ~S, lower-bound ~S~%" node (and node (avl-node-key node) lower-bound))
...
;      (inform "before visit ~S" tree)
      (when (avl-tree-root tree)
	(visit (avl-tree-root tree)))
;      (inform "after visit ~S" tree)
      (cdr result))))
