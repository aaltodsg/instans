...
(defun var-name (node var)
  (let ((from (reverse-resolve-binding (node-instans node) var)))
    (format nil "~A &rarr; ~A" (sparql-var-name from) (sparql-var-name var))))
...
;	(format nil "{<n> ~A| 00000<p>}" (dot-node-pretty-name this))
  ...
  (:method ((this join-node) &key binding-info-box-p &allow-other-keys)
    ;;; TODO: lisää boxi token-storen kokoa varten
    (if binding-info-box-p
	(format nil "<TABLE BORDER=\"0\" CELLBORDER=\"0\" CELLSPACING=\"0\" CELLPADDING=\"1\"><TR><TD COLSPAN=\"2\">~A</TD></TR><TR><TD>&beta;: no items&nbsp;&nbsp;</TD><TD>&nbsp;&nbsp;&alpha;: no items</TD></TR></TABLE>" (dot-node-pretty-name this))
;	(format nil "{<n> ~A| 00000<p>}" (dot-node-pretty-name this))
	(format nil "~A" (dot-node-pretty-name this))))
...
(defun print-dot (net &key (stream *standard-output*) show-vars-p (html-labels-p t) (node-shape "ellipse") binding-info-box-p (horizontalp t))
  (let* ((nodes (instans-nodes net))
	 (alphas (filter #'(lambda (node) (typep node 'alpha-node)) nodes))
	 (alphamems (filter #'(lambda (node) (and (typep node 'alpha-memory) (typep (first (node-parents node)) 'triple-pattern-node))) nodes))
	 (other-nodes (list-difference nodes (list-union alphas alphamems))))
    (format stream "~%digraph ~S{ ~A" (instans-name net) (if horizontalp "rankdir=\"LR\";" ""))
	...
	;; (inform "bef ~S" node)
	;; (describe node)
	;; (inform "parent-slots ~S" (node-parent-slots node))
	;; (loop for ps in (node-parent-slots node)
	;;       do (describe (slot-value node ps)))
	(loop with parent-slots = (node-parent-slots node)
	      for parent-slot in parent-slots
	      for parent = (slot-value node parent-slot)
	      for anchor-point in (if (= 2 (length parent-slots)) (cond ((not horizontalp) '(":nw" ":ne")) (t '(":sw" ":nw"))) '(""))
	      for vars = nil ;;; (and (eq parent-slot 'alpha) (mapcar #'(lambda (var) (format nil "~A" (var-name node var))) (node-def (join-alpha node))))
	      when parent
	      do (format stream "~%  ~A -> ~A~A~A;" (dot-node-name parent) (dot-node-name node) anchor-point (edge-attrs vars parent))
	      ;; do (inform "end ~A ~A ~A" parent-slot parent vars)
	     )
	;; (inform "af ~S" node)
)
...
;    (inform "(directoryp ~A) = ~A" html-file (directoryp html-file))
	...
	(let* (
;; 	       (rules (read-from-url-or-file rules-file))
...
;; </html>"
	  ...
	  (delete-file dot-output-file)
	  (delete-file dot-svg-output-file)
	  ))))
  t)
