...
(defun compile-sparql-file (file &key instans instans-name output-directory (mkhtml-script "mk-html1"))
  (with-open-file (input-stream file)
    (apply #'compile-sparql-stream input-stream :input-name file :instans instans :instans-name instans-name :output-directory output-directory :mkhtml-script mkhtml-script)))

(defun compile-sparql-stream (stream &key input-name instans instans-name output-directory (mkhtml-script "mk-html1") (parser #'sparql-parse-stream))
  (declare (special *node-color-alist*))
  (setf *node-color-alist* nil)
  (when (null instans)
    (setf instans (make-instance 'instans :name instans-name)))
  (let* ((parsing (funcall parser instans stream))
...
(defun build-and-execute-sparql-system (rules-file triples-file &key (report-function ) (report-function-arguments ) (output-directory "/Users/enu/instans/tests/output"));  (show-turtle-parse-p nil))
  ...
  (let* ((instans nil))
    (with-open-file (triples-stream triples-file)
      ...
      (let* ((triples-lexer (make-turtle-lexer triples-stream instans))
...
(defun instans-add-rules (instans-iri rules &key output-directory report-function report-function-arguments)
	  ...
	  (t
	   (let ((string (read-from-url-or-file rules)))
	     ...
	     (with-input-from-string (stream string)
	       ...
	       (compile-sparql-stream stream :instans instans :input-name (if (stringp rules) rules (rdf-iri-string rules)) :output-directory output-directory)
...
(defun instans-add-triples-from-url (instans-iri triples graph)
  (let ((instans (get-instans instans-iri)))
    (cond ((sparql-error-p instans) instans)
	  (t
