...
(defun sparql-value-type-iri (x)
  (cond ((typep x 'xsd-string-value) *xsd-string-iri*)
	((typep x 'xsd-boolean-value) *xsd-boolean-iri*)
...
;	  ((typep x 'xsd-float-value) *xsd-float-iri-string*)
	...
	((rdf-literal-p x)
	 (cond ((rdf-literal-type x) (rdf-literal-type x))
	       (t *xsd-string-iri*)))
	(t (signal-sparql-error "Not a valid SPARQL value: ~A" x))))
...
(defun sparql-value-to-string (x &key instans always-use-typed-literal-p)
  (labels ((as-typed-literal (str type-iri)
	   ...
	   (clean-decimal-string (str)
	     (let ((di (position #\d str :test #'char=)))
	       (cond ((null di) str)
		     ((and (= di (- (length str) 2)) (char= #\0 (char str (1- (length str)))))
		      (subseq str 0 di))
		     (t (setf (char str di) #\e)
			str)))))
    (cond ((typep x 'xsd-string-value) (format nil "\"~A\"" x))
	  ...
	  ((typep x 'xsd-decimal-value) (maybe-as-typed-literal (clean-decimal-string (format nil "~F" x)) *xsd-decimal-iri*))
...
(defun sparql-value-to-sexp (x &key instans)
  (labels ((as-typed-literal (str type-iri)
	     (format nil "\"~A\"^^~A" str (sparql-value-to-string type-iri :instans instans)))
	   (clean-decimal-string (str)
	     (let ((di (position #\d str :test #'char=)))
	       (cond ((null di) str)
		     ...
		     (t (setf (char str di) #\e)
			str)))))
    (cond ((typep x 'xsd-string-value) `(parse-xsd-string ,(format nil "\"~A\"" x)))
	  ((typep x 'xsd-boolean-value) `(parse-xsd-boolean ,(if x "true" "false")))
	  ((typep x 'xsd-integer-value) `(parse-xsd-integer ,(format nil "~D" x)))
	  ((typep x 'xsd-decimal-value) `(parse-xsd-decimal ,(format nil "~F" x)))
	  ((typep x 'xsd-double-value) `(parse-xsd-double ,(clean-decimal-string (format nil "~A" x))))
	  ((typep x 'xsd-datetime-value) `(parse-xsd-datetime ,(as-typed-literal (datetime-canonic-string x) *xsd-datetime-iri*)))
	  ((rdf-literal-p x)
	   (cond ((rdf-literal-type x)
		  `(create-rdf-literal-with-type ,(rdf-literal-string x) (parse-iri ,(iri-to-string (rdf-literal-type x)))))
		 ((rdf-literal-lang x)
		  `(create-rdf-literal-with-lang ,(rdf-literal-string x) ,(rdf-literal-lang x)))
		 (t
		  `(parse-xsd-string ,(format nil "\"~A\"" (rdf-literal-string x))))))
	  ((rdf-iri-p x) `(parse-iri ,(iri-to-string x)))
	  ((rdf-blank-node-p x) `(make-named-blank-node (current-instans) ,(uniquely-named-object-name x)))
	  ((sparql-unbound-p x) `(make-instance 'sparql-unbound))
	  (t (format nil "~A" x)))))

(defun rdf-statement-creating-form (statement)
  (destructuring-bind (s p o g) statement
    `(list ,(sparql-value-to-sexp s) ,(sparql-value-to-sexp p) ,(sparql-value-to-sexp o) ,@(if g (list (sparql-value-to-sexp g))))))
