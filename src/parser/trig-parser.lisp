...
(defun make-trig-parser (instans input-stream &key base subscribe triple-callback triples-block-callback)
  (when (null base) (setf base (parse-iri "http://")))
  (let* ((lexer (make-instance 'trig-lexer :input-stream input-stream :instans instans :base base :show-parses-p subscribe))
	 (triples (list nil))
...
(defun parse-trig-file (file &key subscribe base triple-callback triples-block-callback)
  (let ((*triple-count* 0)
	(*triple-sizes* 0))
    (time
     (with-open-file (stream file)
       (let* ((instans (make-instance 'instans :name file))
	      (parser (make-trig-parser instans stream :base base :subscribe subscribe :triples-block-callback triples-block-callback :triple-callback triple-callback))
...
(defun parse-trig-file-single-triple (file &key subscribe base)
  (time
   (with-open-file (stream file)
     (let* ((instans (make-instance 'instans :name file))
	    (parser (make-trig-parser instans stream :base base :subscribe subscribe
...
(defun parse-trig-file-triples-block (file &key subscribe base)
  (time
   (with-open-file (stream file)
     (let* ((instans (make-instance 'instans :name file))
	    (parser (make-trig-parser instans stream :base base :subscribe subscribe
...
(defun parse-trig-file-both (file &key subscribe base)
  (time
   (with-open-file (stream file)
     (let* ((instans (make-instance 'instans :name file))
	    (parser (make-trig-parser instans stream :base base :subscribe subscribe
