...
;		 (inform "(translate-not-implemented-yet ~S ~S) -> ~S" fmt args msg)
	     ...
	     (equal-value (v1 v2)
	       (let ((r 
	       (or (equal v1 v2)
		   (cond ((instans-var-p v1)
			  (and (instans-var-p v2) (instans-var-equal v1 v2)))
...
;			     (inform "translate-satr: bind-form ~A = ~A" expr form)
		   ...
		   ;; 			       collect (instans-var-name (cdr (find-if #'(lambda (binding) (equal (instans-var-name (first binding)) ts)) (instans-bindings instans))))
		   ;; 			       else collect ts)))
		   ;;    (make-or-share-instance 'service-node :prev prev
		   ;; 			      :silentp (not (null (second args)))
		   ;; 			      :endpoint (third args)
...
(defun collect-blanks (expr &optional blanks)
  (cond ((rdf-blank-node-p expr)
	 (if (member expr blanks :test #'instans-var-equal) blanks (cons expr blanks)))
	((consp expr)
...
(defun translate-triples-op-template (instans template op instans-var allow-blanks-p)
  (let* ((blanks (collect-blanks template))
	 (blank-var-alist (loop for blank in blanks collect (cons blank (gensym (string (instans-var-name blank))))))
	 (iri-vars nil)
	 (literal-vars nil)
	 (triple-op-forms nil))
