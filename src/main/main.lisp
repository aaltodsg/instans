	     ...
	     (progn
	       (loop for (key value) in configuration
		     ...
		     ;;; "base=http://example.org/friends/&graph=http://instans.org/events/&file=tests/input/fnb.ttl&input-policy=triples-block&operations:add:execute:remove:execute"
			  ...
			  (:rdf-input-unit (set-policy :rdf-input-unit (intern value :keyword) (instans-available-query-input-policies instans)))
			  ...
			  (:allow-rule-instance-removal-p (set-policy :allow-rule-instance-removal-p (intern value :keyword) (instans-available-rule-instance-removal-policies instans)))
