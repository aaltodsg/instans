...
(defmethod initialize-instance :after ((this query-output-processor) &key output-name output-stream &allow-other-keys)
  (cond ((not (null output-name))
	 (setf (query-output-processor-output-stream this) (open output-name :direction :output :if-exists :supersede)))
	((null output-stream)
	 (setf (query-output-processor-output-stream this) *standard-output*))))
...
(defun create-query-output-processor (output-name output-type)
  (case output-type
    (:csv (make-instance 'query-output-csv-processor :output-name output-name :output-type output-type))
    (:solution-set (make-instance 'query-output-solution-set-processor :output-name output-name :output-type output-type))
...
(defgeneric write-query-output (query-output-processor node token)
  (:method ((this query-output-csv-processor) node token)
  ...
  (:method ((this query-output-solution-set-processor) node token)
    (multiple-value-bind (vars values) (solution-bindings node token)
      (unless (query-output-processor-headers-written-p this)
	(setf (query-output-processor-headers-written-p this) t)
	(setf (query-output-solution-set-processor-variables this)
	      (mapcar #'(lambda (var) (format nil "~(~A~)" (subseq (uniquely-named-object-name (reverse-resolve-binding (node-instans node) var)) 1))) vars))
	(setf (query-output-solution-set-processor-end this) (setf (query-output-solution-set-processor-bindings this) (list nil))))
      (setf (query-output-solution-set-processor-end this)
	    (setf (cdr (query-output-solution-set-processor-end this)) (list values))))))
...
(defgeneric close-query-output-processor (query-output-processor)
  (:method ((this query-output-csv-processor))
    (when (and (query-output-processor-output-name this) (query-output-processor-output-stream this))
      ...
      (setf (query-output-processor-output-stream this) nil)))
  (:method ((this query-output-solution-set-processor))
    (pop (query-output-solution-set-processor-bindings this))))
