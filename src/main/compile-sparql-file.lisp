...
;	  do (barf "Type of node ~A = ~A, filter-node-p ~A" node  (type-of node) (filter-node-p node))
	  do (cond ((filter-node-p node)
		    (barf "before compile filter-test ~A = ~S, node-use = ~A" node (filter-test node) (node-use node))
		    (let ((filter-lambda `(lambda ,(sparql-var-lisp-names (node-use node)) ;(let ((v 
		      ...
		      (barf "after compile filter-test-lambda ~A = ~S" node (filter-test-lambda node))
		      (setf (filter-test-func node) (compile nil filter-lambda))))
		   ((bind-node-p node)
;		    (barf "bind-form ~A = ~S" node (bind-form node))
...
;		    (barf "aggregate-join-group-form ~A = ~S" node (aggregate-join-group-form node))
...
(defun build-system-from-rules (&rest keys &key network rules-file (output-dir "parser/output") overriding-keys &allow-other-keys)
  ...
  (barf "Adding rules from ~A~%" rules-file)
...
(defun report-execution (network)
  (let ((queue (network-rule-instance-queue network)))
    (barf "Read ~D inputs~&Added ~D quads~&Removed ~D quads~&Executed ~D rules: ~D select rules, ~D modify rules, and ~D construct rules"
