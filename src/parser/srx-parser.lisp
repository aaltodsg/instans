...
(defun parse-srx-stream (instans stream input-name &rest keys &key (errors-in-literal-allowed-p t) &allow-other-keys)
  ...
  (let ((sparql-iri (parse-iri "http://www.w3.org/2005/sparql-results#sparql"))
    ...
    (labels ((srx-error (fmt &rest args) (apply #'error* (concatenate 'string "~A: " fmt) input-name args))
	       ...
	       (cond ((consp x)
		      (cond ((and (stringp (car x)) (stringp (cdr x)))
			    ...
			    (t
			     (let ((tr-car (tr (car x)))
				   (tr-cdr (tr (cdr x))))
			       ;; (inform "~VTdispatching ~S, tr-car = ~S, tr-cdr = ~S" indent x tr-car tr-cdr)
		 ...
		 ;;     )
		 ;; (decf indent 4)
		 ;; (inform "~VTexit tr ~S -> ~S" indent x r)
		 ;; r))
		 ...
		 (cond ((sparql-boolean-result-p (third args))
			...
			(apply #'make-instance 'sparql-query-results :boolean (third args) head-args))
		       ((sparql-results-p (third args))
			(let ((results (sparql-results-results (third args))))
	     ...
	     (translate-results (args orig-form)
	       (unless (and (every #'(lambda (a) (and (member (first a) '("ordered" "distinct") :test #'equal) (member (second a) '("true" "false") :test #'equal))) (first args))
			    (every #'sparql-result-p (rest args)))
		 (srx-error "Illegal results:~%translated ~S~%parsed     ~S" args orig-form))
	       (create-sparql-results (first args) (rest args)))
