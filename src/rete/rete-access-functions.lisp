...
(defgeneric node-defines-vars (node)
  ...
  (:method ((this bind-node))
    (let ((bvar (bind-variable this)))
      (when (find-sparql-var bvar (node-def-preceq (node-prev this)))
  ...
  (:method ((this solution-modifiers-node))
	...
	(loop for item in (solution-modifiers-project-vars this)
	      ...
	      do (let ((var (second item)))
		   (when (find-sparql-var var seen-vars)
    ...
    (labels ((visit (node)
		 ...
		 (let ((precs (node-effective-precs node)))
		   ...
		   (let ((vars-in (reduce #'list-union (mapcar #'node-vars-out precs) :initial-value nil)))
		     ...
		     ;;   (inform "~%visit ~S~%defs ~S~%uses ~S~%vars-add ~S~%vars-del ~S~%vars-in ~S~%vars-out ~S" node (mn (node-def node)) (mn (node-use node)) (mn (node-vars-add node)) (mn (node-vars-del node)) (mn (node-vars-in node)) (mn (node-vars-out node))))
		     )))))
      (loop for node in nodes do (visit node))))
  nodes)
