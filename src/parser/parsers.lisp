...
(defun parse-triple-file (file &key subscribe base triple-callback block-callback document-callback)
  (let* ((*triple-count* 0)
	 (file-type (intern (string-upcase (pathname-type (parse-namestring file))) :keyword))
	 ;; (*triple-sizes* 0)
	 (make-parser (cond ((member file-type '(:ttl :turtle)) #'make-turtle-parser)
			    ((eq file-type ':trig) #'make-trig-parser)
			    ((member file-type '(:nt :ntriples :n-triples)) #'make-n-triples-parser)
			    ((member file-type '(:nq :nquads :n-quads)) #'make-n-quads-parser)
			    (t (error* "Cannot parse files of type ~S" file-type)))))
    (when (and (or block-callback base) (not (member file-type '(:trig :ttl :turtle))))
      (error* "You cannot use block-callback with ~A input" file-type))
;    (time
