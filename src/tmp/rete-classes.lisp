...
(define-class new-query-output-processor () 
  ((output-name :accessor query-output-processor-output-name :initarg :output-name :initform nil)
   (output-writers :accessor query-output-processor-output-writers :initarg :output-writers)))

(define-class new-query-output-writer () ())

(define-class new-stream-query-output-writer (new-query-output-writer)
  ((stream :accessor new-stream-query-output-writer-stream :initarg :stream :initform nil)))

(define-class new-agent-query-output-writer (new-query-output-writer)
  ((agents :accessor new-agent-query-output-writer-agents :initarg :agents :initform nil)))

(define-class new-construct-output-processor (new-query-output-processor) ())

(define-class new-n-statement-output-processor (new-construct-output-processor)
  ((statements :accessor new-n-statement-output-processor-statements :initform nil)
   (tail :accessor new-n-statement-output-processor-tail :initform nil)))

(define-class new-nt-output-processor (new-n-statement-output-processor) ())
(define-class new-nq-output-processor (new-n-statement-output-processor) ())

(define-class new-trig-output-processor (new-construct-output-processor)
  ((current-graph :accessor new-trig-output-processor-current-graph :initform nil)
   (current-subject :accessor new-trig-output-processor-current-subject :initform nil)
   (spol-trie :accessor new-trig-output-processor-spol-trie :initform nil)))

(define-class new-turtle-output-processor (new-trig-output-processor) ())

(defgeneric add-statement (new-construct-output-processor s p o &optional g)
  (:method ((this new-n-statement-output-processor) s p o &optional g)
    (when (typep this 'new-nt-output-processor)
      (assert* (null g) "Non-null graph in N-Triples output: ~A" g))
    (cond ((null (new-n-statement-output-processor-statements this))
	   (setf (new-n-statement-output-processor-statements this) (list (list s p o g)))
	   (setf (new-n-statement-output-processor-tail this) (new-n-statement-output-processor-statements this)))
	  (t
	   (setf (cdr (new-n-statement-output-processor-tail this)) (list (list s p o g)))
	   (setf (new-n-statement-output-processor-tail this) (cdr (new-n-statement-output-processor-tail this))))))
  (:method ((this new-trig-output-processor) s p o &optional g)
    (when (typep this 'new-turtle-output-processor)
      (assert* (null g) "Non-null graph in Turtle output: ~A" g))
    (when (null (new-trig-output-processor-spol-trie this))
      (setf (new-trig-output-processor-spol-trie this) (make-instance 'trie)))
    (trie-add-path (new-trig-output-processor-spol-trie this) (list g s p o))))

(defgeneric flush-construct-output (new-construct-output-processor)
  (:method ((this n-statement-output-processor))
    (loop for writer in (query-output-processor-output-writers this)
	  do (write-query-output-statements writer (n-statement-output-processor-statements this)))
    (setf (n-statement-output-processor-statements this) nil)
    (setf (n-statement-output-processor-tail this) nil))
  (:method ((this trig-output-processor))
    (loop for writer in (query-output-processor-output-writers this)
	  do (write-query-output-trie writer (trig-output-processor-spol-trie this)))
    (setf (trig-output-processor-spol-trie this) nil)))

(defgeneric write-query-output-statements (new-query-output-writer statements)
  (:method ((this new-stream-query-output-writer) statements)
    (loop for (s p o g) in statements
	 do (format (new-stream-query-output-writer-stream this) "~&~A ~A ~A~@[ ~A~]~%" s p o g)))
  (:method ((this new-agent-query-output-writer) statements)
    (loop for agent in (new-agent-query-output-writer-agents this)
	  do (agent-send agent statements))))

(defgeneric write-query-output-trie (new-query-output-writer trie)
  (:method ((this new-stream-query-output-writer) trie)
    (let ((stream (new-stream-query-output-writer-stream this))
	  (indent 0)
	  (init-sep ""))
      (loop for (g . s-trie) in (trie-level trie)
	    do (let (g-string)
		 ...
		 (loop for (s . p-trie) in (trie-level s-trie)
			 ...
			 do (incf indent (- (length s-string)))
			 do (format stream " .~%"))))))
  (:method ((this new-agent-query-output-writer) trie)
    (let* ((msg (list nil))
	   (tail msg))
